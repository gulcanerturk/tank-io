...
setInterval(() => {
    ...
    for (const [sid, p] of Object.entries(players)) {
      ...
      if (dist(b, p) < b.radius + p.radius) {
        ...
        if (p.health <= 0) {
          ...
          if (killer) {
            killer.score += 200;
            killer.upgradePoints += 3;
            gainXP(killer, 200);
            io.to(b.ownerId).emit('killedPlayer', { name: p.name });
          }
  ...
  for (const p of Object.values(players)) {
    ...
    if (p.health < maxHp) p.health = Math.min(maxHp, p.health + 0.05);
  }
  ...
  io.emit('gameState', {
    players: Object.fromEntries(
      Object.entries(players).map(([id, p]) => [id, {
        x: p.x, y: p.y, angle: p.angle, health: p.health,
    ...
    bullets: bullets.map(b => ({ id: b.id, x: b.x, y: b.y, radius: b.radius, color: b.color, ownerId: b.ownerId })),
    shapes: shapes.map(s => ({ id: s.id, x: s.x, y: s.y, type: s.type, radius: s.radius, color: s.color, angle: s.angle, health: s.health, maxHealth: s.maxHealth }))
  });
...
io.on('connection', (socket) => {
  ...
  players[socket.id] = {
    x: Math.random() * 800 + WORLD.w/2 - 400,
    y: Math.random() * 800 + WORLD.h/2 - 400,
    radius: 22, angle: 0,
  ...
  socket.on('setName', (name) => {
    if (players[socket.id]) {
      players[socket.id].name = String(name).slice(0, 16) || 'Oyuncu';
    }
  });
  ...
  socket.on('input', (data) => {
    ...
    p.x += data.dx * st.speed * diag;
    p.y += data.dy * st.speed * diag;
    ...
    if (data.shooting) {
      ...
      if (now - p.lastShot >= st.fireInterval) {
        ...
        for (let b = 0; b < st.bulletCount; b++) {
          ...
          bullets.push({
            id: bulletId++,
            x: p.x + Math.cos(ang) * barrelLen,
            y: p.y + Math.sin(ang) * barrelLen,
            vx: Math.cos(ang) * st.bulletSpeed,
            vy: Math.sin(ang) * st.bulletSpeed,
            radius: 7 + (p.upgrades.bulletDamage || 0),
            damage: st.bulletDamage,
            color: p.color,
            life: 70,
            ownerId: socket.id
          });
  ...
  socket.on('upgrade', (stat) => {
    ...
    const MAX = 7;
    if ((p.upgrades[stat] || 0) >= MAX) return;
    p.upgrades[stat] = (p.upgrades[stat] || 0) + 1;
    p.upgradePoints--;
  ...
  socket.on('respawn', () => {
    const p = players[socket.id];
    if (!p) return;
    p.x = Math.random() * 800 + WORLD.w/2 - 400;
    p.y = Math.random() * 800 + WORLD.h/2 - 400;
    p.health = 100;
    p.alive = true;
